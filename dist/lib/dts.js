...
function fixDtsImportPaths(context, dtsAST, schemaDtsFullPath, typeInjectEntrypointDir) {
            ...
            if (path.node.source.value.endsWith(paths_1.SCHEMA_TYPES_BASENAME)) {
                switch (type) {
                    case 'document-import':
                        path.node.source = types_1.stringLiteral(path_1.join(typeInjectEntrypointDir, '__generated__/__types__'));
                        break;
                    case 'gql-call':
                    case 'load-call':
...
function decorateDts(context, dtsContent, schemaDtsFullPath, typeInjectEntrypointDir) {
    const { type } = context;
    ...
    switch (type) {
        ...
        case 'document-import':
            ...
            fixDtsImportPaths(context, dtsAST, schemaDtsFullPath, typeInjectEntrypointDir);
    }
...
async function processDtsForContext(execContext, codegenContext) {
    ...
    const typeInjectEntrypointDir = path_1.dirname(execContext.config.typeInjectEntrypoint);
    ...
    for (const [i, dtsContent] of dtsContents.entries()) {
        ...
        let content = decorateDts(ctx, dtsContent, schemaDtsFullPath, typeInjectEntrypointDir);
